...
namespace _4OF.ee4v.AssetManager {
    public class TestAssetManagerWindow : EditorWindow {
        ...
        private void OnEnable() {
            ...
            if (_repository == null) {
                _logs.Add("Repository is not initialized.");
            }
        ...
        private void OnGUI() {
            if (_repository == null) {
                if (GUILayout.Button("Reload Container")) {
                    AssetManagerContainer.Reload();
                    OnEnable();
                }
                return;
            }
        ...
        private void DrawLeftPanel() {
            ...
            if (GUILayout.Button("Load")) {
                _repository.Load();
                _logs.Add("Load called.");
            }
            
            if (GUILayout.Button("Verify Cache")) {
                _ = _repository.LoadAndVerifyAsync();
                _logs.Add("LoadAndVerifyAsync invoked.");
            }
            
            if (GUILayout.Button("Container Reload")) {
                AssetManagerContainer.Reload();
                OnEnable();
                _logs.Add("Container Reloaded.");
            }
            ...
            var loaded = libMetadata != null;
            
            EditorGUILayout.LabelField($"Loaded: {loaded}");
            EditorGUILayout.LabelField($"Assets: {assets.Count}");
            
            // タグ集計（リポジトリから全件取得して集計）
            var allTags = assets.SelectMany(a => a.Tags).Distinct().Count();
            EditorGUILayout.LabelField($"Tags: {allTags}", GUILayout.Width(180));
            
            var metadataFolderCount = libMetadata == null ? 0 : CountMetadataFolders(libMetadata);
            ...
            if (GUILayout.Button("Create", GUILayout.Width(80)))
                try {
                    if (string.IsNullOrEmpty(_createAssetFilePath)) {
                        ...
                        _assetService.CreateAsset(_createAssetFilePath);
            ...
            if (GUILayout.Button("Set Thumbnail", GUILayout.Width(120))) {
                if (_selectedAssetId == Ulid.Empty) {
                    ...
                    _repository.SetThumbnail(_selectedAssetId, _thumbnailFilePath);
            ...
            if (GUILayout.Button("Remove Thumbnail", GUILayout.Width(120))) {
                if (_selectedAssetId == Ulid.Empty) {
                    ...
                    _repository.RemoveThumbnail(_selectedAssetId);
            ...
            if (GUILayout.Button("Create Folder", GUILayout.Width(120))) {
                ...
                try {
                    _folderService.CreateFolder(parentId, _newFolderName, _newFolderDescription);
            ...
            if (libMetadata == null)
                ...
                foreach (var rootf in libMetadata.FolderList)
            ...
            if (GUILayout.Button("Rename Folder", GUILayout.Width(120))) {
                if (_selectedFolder != Ulid.Empty) {
                    _folderService.SetFolderName(_selectedFolder, _folderRenameValue);
            ...
            if (GUILayout.Button("Move Folder", GUILayout.Width(120))) {
                if (_selectedFolder == Ulid.Empty) {
                    ...
                    _folderService.MoveFolder(_selectedFolder, parentId);
            ...
            if (GUILayout.Button("Set Desc", GUILayout.Width(80))) {
                if (_selectedFolder == Ulid.Empty) {
                    ...
                    _folderService.SetFolderDescription(_selectedFolder, descTmp);
            ...
            if (GUILayout.Button("Update Folder", GUILayout.Width(120))) {
                if (_selectedFolder == Ulid.Empty) {
                    ...
                    var existing = _repository.GetLibraryMetadata()?.GetFolder(_selectedFolder);
                    if (existing == null) {
                        ...
                        if (existing is Folder existingFolder) {
                            ...
                            _folderService.UpdateFolder(updated);
        ...
        private void DrawRightPanel() {
            ...
            var currentAssets = _repository.GetAllAssets().ToList();
            ...
            foreach (var a in currentAssets) {
                ...
                EditorGUILayout.LabelField(string.Join(",", a.Tags ?? new List<string>().AsReadOnly()), GUILayout.Width(200));
        ...
        private void DrawSelectedAssetControls() {
            ...
            var asset = _repository.GetAsset(_selectedAssetId);
            if (asset == null) {
            ...
            if (GUILayout.Button("Rename"))
                try {
                    _assetService.SetAssetName(_selectedAssetId, _renameValue);
            ...
            if (GUILayout.Button("Set Description"))
                try {
                    _assetService.SetDescription(_selectedAssetId, _descriptionValue);
            ...
            if (GUILayout.Button("Clear Booth Data", GUILayout.Width(140)))
                try {
                    ...
                    _assetService.UpdateAsset(current);
            ...
            if (GUILayout.Button("Item")) {
                _assetService.SetBoothItemId(_selectedAssetId, _boothUrlValue);
                _logs.Add("SetBoothItemId invoked.");
            }

            if (GUILayout.Button("Shop")) {
                _assetService.SetBoothShopDomain(_selectedAssetId, _boothUrlValue);
                _logs.Add("SetBoothShopName invoked.");
            }

            if (GUILayout.Button("Download")) {
                _assetService.SetBoothDownloadId(_selectedAssetId, _boothUrlValue);
            ...
            if (GUILayout.Button("Set Folder", GUILayout.Width(120))) {
                if (Ulid.TryParse(_folderValue, out var uuid)) {
                    _assetService.SetFolder(_selectedAssetId, uuid);
            ...
            if (GUILayout.Button("Add", GUILayout.Width(60))) {
                _assetService.AddTag(_selectedAssetId, _tagValue);
                _logs.Add($"AddTag: {_tagValue}");
            }

            if (GUILayout.Button("Remove", GUILayout.Width(60))) {
                _assetService.RemoveTag(_selectedAssetId, _tagValue);
            ...
            if (!asset.IsDeleted) {
                if (GUILayout.Button("Soft Delete (Trash)", GUILayout.Width(160))) {
                    _assetService.RemoveAsset(_selectedAssetId);
                ...
                if (GUILayout.Button("Permanent Delete", GUILayout.Width(140))) {
                    var ok = EditorUtility.DisplayDialog("Confirm Permanent Delete",
                        ...
                        "Delete", "Cancel");
                    if (ok) {
                        _assetService.DeleteAsset(_selectedAssetId);
                ...
                if (GUILayout.Button("Restore", GUILayout.Width(120))) {
                    _assetService.RestoreAsset(_selectedAssetId);
        ...
        private void RefreshSelectedFields() {
            if (_selectedAssetId != Ulid.Empty) {
                var a = _repository.GetAsset(_selectedAssetId);
                if (a != null) {
                    _renameValue = a.Name;
                    _descriptionValue = a.Description;
                    // prefill booth url input
                    if (a.BoothData != null)
        ...
        private int CountFolderChildren(Folder folder) {
            ...
            foreach (var child in folder.Children) {
                c += 1;
                if (child is Folder childFolder) c += CountFolderChildren(childFolder);
            }
            return c;
        }
    }
