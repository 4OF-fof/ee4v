...
namespace _4OF.ee4v.AssetManager.Adapter {
	public static class HttpServer {
		...
		private static void ListenLoop() {
			...
			while (IsRunning) {
				try {
					var context = _listener.GetContext(); // Will block until a request comes in or the listener is closed
					try {
						HandleContext(context);
					}
					catch (Exception e) {
						Debug.LogError("Error handling request: " + e);
						try { context.Response.StatusCode = 500; context.Response.Close(); } catch (Exception ex) { Debug.LogWarning("Failed closing response after exception: " + ex); }
					}
				}
		...
		private static void HandleContext(HttpListenerContext ctx) {
			...
			try {
				if (req.HttpMethod.Equals("GET", StringComparison.OrdinalIgnoreCase) && (req.Url.AbsolutePath == "/health" || req.Url.AbsolutePath == "/health/")) {
					var uptime = (DateTime.UtcNow - _startedAt).TotalSeconds;
					var body = $"{{\"status\":\"ok\", \"uptimeSeconds\": {uptime:F1}, \"startedAt\": \"{_startedAt:o}\" }}";
				...
				else if (req.HttpMethod.Equals("POST", StringComparison.OrdinalIgnoreCase) && (req.Url.AbsolutePath == "/contents" || req.Url.AbsolutePath == "/contents/")) {
					...
					if (string.IsNullOrWhiteSpace(body)) {
						...
						resp.ContentLength64 = empty.Length; resp.OutputStream.Write(empty, 0, empty.Length);
					}
					else {
						...
						if (wrapper?.Shops == null) {
							...
							resp.ContentLength64 = err.Length; resp.OutputStream.Write(err, 0, err.Length);
						}
						else {
							...
							resp.ContentLength64 = ok.Length; resp.OutputStream.Write(ok, 0, ok.Length);
						}
					}
				}
				else {
