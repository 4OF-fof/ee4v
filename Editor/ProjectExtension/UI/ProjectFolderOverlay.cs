    ...
    public static class ProjectFolderOverlay {
        public static void Draw(string path, Rect selectionRect) {
            ...
            if (EditorPrefsManager.EnableStyledFolder && selectionRect.Contains(e.mousePosition) && e.alt) {
                ...
                var selectedPathList = Selection.assetGUIDs.Select(AssetDatabase.GUIDToAssetPath).Where(AssetDatabase.IsValidFolder).ToList();
            ...
            if (selectionRect.height > 20) {
                imageRect = new Rect(selectionRect.x - 1, selectionRect.y - 1, selectionRect.width + 2, selectionRect.width + 2);
            }
            else if (selectionRect.x > 20) {
                imageRect = new Rect(selectionRect.x - 1, selectionRect.y - 1, selectionRect.height + 2, selectionRect.height + 2);
                backgroundColor = ColorPreset.DefaultBackground;
            }
            else {
                imageRect = new Rect(selectionRect.x + 2, selectionRect.y - 1, selectionRect.height + 2, selectionRect.height + 2);
            }

            var isDrawIcon = false;
            if (EditorPrefsManager.EnableStyledFolder) {
                isDrawIcon = DrawStyledFolder(path, imageRect, backgroundColor);
            }

            if (EditorPrefsManager.ShowFolderOverlayIcon && !isDrawIcon) {
        ...
        private static bool DrawStyledFolder(string path, Rect imageRect, Color backgroundColor) {
            ...
            var folderIcon = isEmpty ? EditorGUIUtility.IconContent("FolderEmpty Icon").image : EditorGUIUtility.IconContent("Folder Icon").image;
        ...
        private static void DrawOverlayIcon(string path, Rect imageRect) {
            var overlayRect = new Rect((imageRect.x + imageRect.xMax) / 2, (imageRect.y + imageRect.yMax) / 2, imageRect.width / 2, imageRect.height / 2);
            var overlayIcon = GetMostIconInFolder(path);
            if (overlayIcon == null) return;
            var outlineOffsetsOuter = new[] { new Vector2(-1f, 0), new Vector2(1f, 0), new Vector2(0, -1f), new Vector2(0, 1f) };
        ...
        private static Texture GetMostIconInFolder(string path) {
            ...
            foreach (var assetPath in childAssetPaths) {
                if (AssetDatabase.IsValidFolder(assetPath) || Path.GetExtension(assetPath) == ".meta") {
                    continue;
                }

                var asset = AssetDatabase.LoadMainAssetAtPath(assetPath);
                var icon = asset is Texture ? EditorGUIUtility.IconContent("Texture Icon").image : AssetPreview.GetMiniThumbnail(asset);
                if (icon == null) continue;
                if (!iconCounts.TryAdd(icon, 1)) {
                    iconCounts[icon]++;
                }
            }

            return iconCounts.Count == 0 ? null :
                iconCounts.OrderByDescending(kv => kv.Value).FirstOrDefault().Key;
        }
    }
}