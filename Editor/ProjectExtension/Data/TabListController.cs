...
namespace _4OF.ee4v.ProjectExtension.Data {
    public static class TabListController {
        ...
        public static VisualElement CurrentTab() => _currentTab;

        public static void Initialize() {
            if (_asset == null) {
                _asset = TabListObject.LoadOrCreate();
            }
        ...
        public static void Move(int fromIndex, int toIndex) {
            ...
            if (_currentTab == tab) {
                Tab.SetState(tab, Tab.State.Selected);
            }
            Save();
        }
        ...
        private static void Sync() {
            ...
            var existingTabs = _tabContainer.Children().Where(e => e.name == "ee4v-project-toolbar-tabContainer-tab").ToList();
            foreach (var t in existingTabs) {
                _tabContainer.Remove(t);
            }
            ...
            var firstElem = _tabContainer.Children().FirstOrDefault(e => e.name == "ee4v-project-toolbar-tabContainer-tab" && e.tooltip == firstPath);
            if (firstElem == null) return;
            Tab.SetState(firstElem, Tab.State.Selected);
            SelectTab(firstElem);
            var others = _tabContainer.Children().Where(e => e.name == "ee4v-project-toolbar-tabContainer-tab" && e.tooltip != firstPath).ToList();
