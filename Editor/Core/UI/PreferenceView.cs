...
namespace _4OF.ee4v.Core.UI {
    public class PreferencesProvider : SettingsProvider {
        ...
        private PreferencesProvider(string path, SettingsScope scopes, IEnumerable<string> keywords) : base(path, scopes, keywords){}
        ...
        public override void OnGUI(string searchContext) {
            ...
            EditorPrefsManager.ShowFolderOverlayIcon = EditorGUILayout.Toggle(
                new GUIContent("Show Folder Overlay Icon", "Show or hide the folder overlay icon in the Project window."),
            ...
            EditorPrefsManager.CompatLilEditorToolbox = EditorGUILayout.Toggle(
                new GUIContent("Compatibility lilEditorToolbox", "Enable this option if you are using lilEditorToolbox to avoid UI conflicts."),
                EditorPrefsManager.CompatLilEditorToolbox);
            
            EditorGUI.BeginChangeCheck();
            EditorPrefsManager.CompatFaceEmo = EditorGUILayout.Toggle(
                new GUIContent("Compatibility FaceEmo", "Hide component icons when a VRChat Avatar Descriptor is present to avoid UI conflicts with FaceEmo."),
            ...
            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField(new GUIContent("Hidden Item List", "Comma-separated list of component type names to hide in the 'Add Component' menu (e.g. Transform, RectTransform)."));
            HiddenItemList();
            ...
            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField(new GUIContent("Ignore Component Name List", "Comma-separated list of component type names to ignore when gathering component icons (e.g. Transform, RectTransform)."));
            EditorGUI.BeginChangeCheck();
            IgnoreComponentName();
            ...
            if (GUILayout.Button("Reset to Default", ignoreResetStyle)) {
                ...
                _ignoreComponentListCache = new List<string>(EditorPrefsManager.IgnoreComponentNameList);
                if (_ignoreComponentReorderableList != null) _ignoreComponentReorderableList.list = _ignoreComponentListCache;
                EditorApplication.RepaintHierarchyWindow();
            }
            if (EditorGUI.EndChangeCheck()) {
                EditorApplication.RepaintHierarchyWindow();
            ...
            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField(new GUIContent("Icon List", "List of icon keys used by the icon selector. Use '<SEP>' (without quotes) to insert a separator. Examples: d_Folder Icon, d_UnityEditor.SceneView."));
            IconListUi();
        ...
        private static void IconListUi() {
            ...
            _iconUseAssetFlags ??= new List<bool>(_iconListCache.Select(s => s != null && s.StartsWith("asset:")));
            while (_iconUseAssetFlags.Count < _iconListCache.Count) _iconUseAssetFlags.Add(false);
            if (_iconUseAssetFlags.Count > _iconListCache.Count) _iconUseAssetFlags.RemoveRange(_iconListCache.Count, _iconUseAssetFlags.Count - _iconListCache.Count);
            if (_iconReorderableList == null) {
                ...
                _iconReorderableList.drawElementCallback = (rect, index, _, _) => {
                    ...
                    var btnContent = toggleIcon != null ? new GUIContent(toggleIcon, tooltip) : new GUIContent(isAsset ? "Icon" : "Texture", tooltip);
                    if (GUI.Button(toggleRect, btnContent)) {
                        isAsset = !isAsset;
                        _iconUseAssetFlags[index] = isAsset;
                        if (isAsset || list[index]?.StartsWith("asset:") == true) {
