...
namespace _4OF.ee4v.HierarchyExtension.Service {
    public static class ReflectionWrapper {
        private static readonly Type SceneHierarchyWindowType = typeof(Editor).Assembly.GetType("UnityEditor.SceneHierarchyWindow");
        private static readonly PropertyInfo LastInteractedHierarchyWindow = SceneHierarchyWindowType?.GetProperty("lastInteractedHierarchyWindow", BindingFlags.Public | BindingFlags.Static);
        ...
        private static void AssignTreeView() {
            ...
            var sceneHierarchy = lastWindow?.GetType().GetField("m_SceneHierarchy", BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(lastWindow);
            if (sceneHierarchy == null) return;

            _treeView = sceneHierarchy.GetType().GetField("m_TreeView", BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(sceneHierarchy);
