...
namespace Ee4v.AssetManager.Infrastructure.Datasources
{
    internal sealed class SqliteDatabaseSnapshot : IDisposable
    {
        private readonly string _snapshotDirectoryPath;

        private SqliteDatabaseSnapshot(string snapshotDirectoryPath, string databasePath)
        {
        ...
        public static SqliteDatabaseSnapshot Create(string sourceDatabasePath, string snapshotKind)
        {
            ...
            var snapshotDirectoryPath = Path.Combine(
                Path.GetTempPath(),
                "ee4v",
                string.IsNullOrWhiteSpace(snapshotKind) ? "sqlite-db-snapshots" : snapshotKind,
            ...
            return new SqliteDatabaseSnapshot(snapshotDirectoryPath, snapshotDatabasePath);
        }
