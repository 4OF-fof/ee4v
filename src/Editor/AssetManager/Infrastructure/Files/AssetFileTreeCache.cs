...
namespace Ee4v.AssetManager.Infrastructure.Files
{
    internal static class AssetFileTreeCache
    {
        ...
        public static IReadOnlyList<AssetArchiveEntry> ReadZipEntries(
        ...
        private static IReadOnlyList<AssetArchiveEntry> CreateDisplayEntries(
            string zipPath,
            IReadOnlyList<AssetArchiveEntry> entries)
        {
            ...
            var displayEntries = new List<AssetArchiveEntry>(entries.Count);
            for (var i = 0; i < entries.Count; i++)
            {
                var displayPath = AssetArchivePathUtility.ToDisplayPath(entries[i].FullName, ignoredRoot);
                if (!string.IsNullOrEmpty(displayPath))
                {
                    displayEntries.Add(new AssetArchiveEntry(displayPath, entries[i].Length, entries[i].ArchiveFullName));
                }
            }
        ...
        private static IReadOnlyList<AssetArchiveEntry> ReadArchive(string zipPath, CancellationToken cancellationToken)
        {
            ...
            using (var archive = new ZipArchive(stream, ZipArchiveMode.Read))
            {
                for (var i = 0; i < archive.Entries.Count; i++)
                {
                    ...
                    entries.Add(new AssetArchiveEntry(entry.FullName, entry.Length));
                }
            }
        ...
        private static bool TryReadCache(
            ...
            FileInfo source,
            CancellationToken cancellationToken,
            out IReadOnlyList<AssetArchiveEntry> entries)
        {
            entries = null;
            ...
            try
            {
                ...
                using (var reader = new BinaryReader(stream, Encoding.UTF8, false))
                {
                    ...
                    var cached = new List<AssetArchiveEntry>(count);
                    for (var i = 0; i < count; i++)
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        cached.Add(new AssetArchiveEntry(reader.ReadString(), reader.ReadInt64()));
                    }
