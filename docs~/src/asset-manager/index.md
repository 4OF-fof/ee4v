...
- Infomation Panel は Item の単一選択時に Asset Info を初期表示する。名前、説明、ユーザー tag は各入力欄から focus が外れた時点で変更がある場合だけ自動保存する。ファイル追加は Item の Asset Info から行える。Variant Group と Version Group の Asset Info は親 Item ではなく選択グループ自身の名前、説明、作成・更新日時を表示して編集し、配下 file 数と origin 種別を表示する。グループでは Item 専用の tag とファイル追加を表示しない。File の単一選択時は tab を表示せず File Tree だけを表示し、Item に属さない File も File ID から内容を取得する。datasource snapshot は編集しない
...
- Main View の読み込み中・空状態・エラーと、archive 詳細の読み込み中・エラーは共通の `ErrorScreen` で内容を隠し、中央の Fluent icon と localized message を表示する。空の `ListView` が持つ標準テキストは画面へ露出させない
