...
- Collection を選択した Main View は、直接所属する Item に加えて直下の子 Collection を `SortOrder` 順で grid の先頭へ表示する。子 Collection card のダブルクリックはその Collection へ遷移し、Item の選択通知や Collection 登録用 drag payload には含めない。Collection snapshot の変更時は検索済み Item cache に子 Collection card だけを再合成し、Item の再検索を行わない
