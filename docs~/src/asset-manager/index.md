...
- Collection を選択した Main View は、直接所属する Item に加えて直下の子 Collection を `SortOrder` 順で grid の先頭へ表示する。子 Collection card だけが内容 thumbnail を最大3件の `ImageStack` stateとして描画し、1件の場合も同じstack container sizeを使う。内容が0件の場合だけ背景を透明にしたCollection iconを表示する。Variant / Version Group は子要素数に関係なく常に種類iconを主表示し、ImageStackを使わない。右下の種類iconは表示せず、Collection / Variant Group / Version Group / fileの種類iconを名前の左へ表示する。Variant Group は Fluent `Folder` と `Branch Fork`、Version Group は Fluent `Folder` と `Layer` を合成した単色iconを88pxで主表示し、ZIP / unitypackage も Fluent `Folder Zip` を88pxで主表示する。その他のarchiveは Fluent `Archive`を使い、SVGから事前生成した透明512px PNGを非圧縮・mipmapなしでimportすることで、大きなcardとHiDPI表示でも十分な解像度と輪郭を保つ。fileの主表示にはitem thumbnailを使わず、ZIPを含めて拡張子に対応するfile iconを表示する。子 Collection card のダブルクリックはその Collection へ遷移し、Item の選択通知や Collection 登録用 drag payload には含めない。Collection snapshot の変更時は検索済み Item cache に子 Collection card だけを再合成し、Item の再検索を行わない
