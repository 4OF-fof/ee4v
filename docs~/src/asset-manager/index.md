...
- Main View の選択中 grid item は通常 Collection 行へドラッグして一括登録できる。登録時は既存の別 Collection 所属を保持し、Smart Collection への直接登録は受け付けない。drag payload は Unity Editor 全体の DragAndDrop を使うため、統合 window 内だけでなく単独 Main View window から単独 Navigation window へのドロップにも対応する。file / group の card をドラッグした場合は、その card が属する Item を登録する。所属変更は専用の `ItemCollections` change を発行し、追加先 Collection または `Uncategorized` を表示中の Main View だけを再描画する。`All` や所属が変わらない別 Collection の Main View は現在の表示を維持し、次の遷移では共有 catalog snapshot から再度 filter する
...
- schema v8、Item/File/Tag/Collection/Dependency/Import Target/Imported Asset GUID・保護 API を実装済み
