...
- `MainViewController` は `MainViewHost` ごとに生成し、その表示セッションの navigation、履歴、grid 列数、file / group 子要素 cache、tag 一覧 cache、非同期 load と cancellation を所有する。catalog item 一覧の条件別 cache は持たない。統合 window と単独 window の controller instance は共有せず、AssetManager Application service の一覧用 catalog snapshot だけを全 window で共有する。All、BOOTH Items、Collection、キーワードの切替は同じ snapshot に対する memory filter とし、Uncategorized は未所属 File を DB から取得する
- `MainView` は AssetManager 固有の UI controller として、検索文字列、一覧 mode、選択中 item などの画面状態を持ち、Core UI component へ描画 state を渡す。file の詳細画面は持たない
...
- Navigation は `All`、Booth snapshot を持つ Item の `BOOTH Items`、Item・Version Group・Variant Group のいずれにも属さない File の `Uncategorized`、Item grid とは別の tag 一覧ページを開く `Tags` を提供する
...
- 通常 Collection の表示アイコンはFluent PNGの塗り付きfolderに固定し、Smart Collection はFluent PNGから選定した30種類を15個×2行で横幅いっぱいに表示するpreset、またはProject内の Texture assetを使用して区別する。Texture選択欄のラベルは「アイコンを選択」とする。Collection 見出しには `+` ボタンを 1 つだけ表示し、クリック時の context menu から通常 Collection または Smart Collection の作成を選ぶ。選択後は枠線付きのアンカー popup を開き、実測したコンテンツ高に合わせて高さを調整し、最大高を超えた場合だけ縦スクロールする。通常 Collection は名前だけを入力し、Smart Collection は名前、アイコン、match mode、条件一覧を入力する。条件追加buttonは左端に`+` iconを表示し、操作後のfocus枠を残さない。Smart Collection の条件 field は名前、説明、タグ、ファイル名、拡張子に限定する
- Collection 行の右クリック context menu から名前変更と削除を行える。右クリックでは現在の Collection 選択を変更しない。複数選択中の選択行から開いた context menu は削除だけを表示し、選択した Collection 群と各子孫を一括削除する。名前変更 popup は右クリック位置を基準に開き、Smart Collection では名前とアイコンを同時に変更できる。Smart Collection では条件変更も表示し、右クリック位置を基準に開く編集 popup へ保存済みの match mode と条件一覧を復元する。削除前には確認を表示し、対象 Collection と配下の子孫 Collection を削除する。Collection への Item 所属は解除するが Item 本体は削除しない
- Collection 作成後は、Smart Collection の場合だけ作成した Collection を Main View で開く。通常 Collection の作成では現在の Main View と履歴を維持する
- Main View の選択中 grid item は通常 Collection 行へドラッグして一括登録できる。登録時は既存の別 Collection 所属を保持し、Smart Collection への直接登録は受け付けない。drag payload は Unity Editor 全体の DragAndDrop を使うため、統合 window 内だけでなく単独 Main View window から単独 Navigation window へのドロップにも対応する。file / group の card をドラッグした場合は、その card が属する Item を登録する。Item に属さない File は Collection へ登録できない。所属変更は専用の `ItemCollections` change を発行し、追加先 Collection を表示中の Main View だけを再描画する
- Collection を選択した Main View は、直接所属する Item に加えて直下の子 Collection を `SortOrder` 順で grid の先頭へ表示する。子 Collection card だけが内容 thumbnail を最大3件の `ImageStack` stateとして描画し、1件の場合も同じstack container sizeを使う。内容が0件の場合だけ背景を透明にしたCollection iconを表示する。Variant / Version Group は子要素数に関係なく常に種類iconを主表示し、ImageStackを使わない。右下の種類iconは表示せず、Collection / Variant Group / Version Group / fileの種類iconを名前の左へ表示する。Variant Group は Fluent `Folder` と `Branch Fork`、Version Group は Fluent `Folder` と `Layer` を合成した単色iconを88pxで主表示し、ZIP / unitypackage も Fluent `Folder Zip` を88pxで主表示する。その他のarchiveとすべてのfile種別、AssetManagerのナビゲーション・ツールバー・編集操作も同じFluent PNG setを使う。SVGから事前生成した透明512px PNGを非圧縮・mipmapなしでimportすることで、大きなcardとHiDPI表示でも十分な解像度と輪郭を保つ。fileの主表示にはitem thumbnailを使わず、ZIPを含めて拡張子に対応するfile iconを表示する。子 Collection card のダブルクリックはその Collection へ遷移し、Item の選択通知や Collection 登録用 drag payload には含めない。Collection snapshot の変更時は現在表示中の raw Item list に子 Collection card だけを再合成し、catalog snapshot の再検索を行わない
...
- `InfomationPanel` は渡された選択 state を描画する。統合 window は同じ layout 内で直接配線し、単独 Main View / Infomation window は Composition が生成する揮発性の standalone view session を介して選択 state を共有する。standalone session は現在値を保持するため、Infomation window を後から開いた場合も現在の選択を復元する
- Windows Explorer の通常 file は表示中の tab や子要素にかかわらず Main View 全域でドロップを受け付ける。統合 AssetManager では Navigation と Information を除いた中央列全体、単独 Main View window では toolbar と余白を含む window 本体全体を受付範囲とする。受け付け中は同じ範囲の上へ半透明の情報色 overlay を表示し、drop、cancel、window 外への移動、panel の detach のいずれでも解除する。file 名、extension、size を実体から補完し、Item 未所属の File と `ee4v_file_origin` を登録して Uncategorized へ移動する。directory と Unity Project 内の相対 path は受け付けない。Item 作成用の toolbar 操作は持たない
- Infomation Panel は Item の単一選択時に Asset Info を初期表示する。名前と説明は各入力欄から focus が外れた時点で変更がある場合だけ自動保存する。ユーザー tag は選択済みの tag を chip で表示し、「新しいタグ」をクリックしたカーソル位置を基準に開く popup から検索、複数選択、新規作成を行う。popup を閉じた時点で変更がある場合だけ自動保存する。ファイル追加は Item の Asset Info から行える。Variant Group と Version Group の Asset Info は親 Item ではなく選択グループ自身の名前、説明、作成・更新日時を表示して編集し、配下 file 数と origin 種別を表示する。グループでは Item 専用の tag とファイル追加を表示しない。File の単一選択時は tab を表示せず File Tree だけを表示し、Item に属さない File も File ID から内容を取得する。datasource snapshot は編集しない
...
- schema v11、Item/File/Tag/Collection/Dependency/Import Target/Imported Asset GUID・保護 API を実装済み
- Main View への外部 file ドロップによる未所属 File と `ee4v` origin の登録、Asset Info での Item 情報編集と追加 file 登録を実装済み
...
- Main View の読み込み中・空状態・エラーと、archive 詳細の読み込み中・エラーは共通の `ErrorScreen` で内容を隠す。短時間の読み込みは中央に localized message だけを表示し、空状態とエラーには Fluent icon を添える。空の `ListView` が持つ標準テキストは画面へ露出させない
...
- Main View grid の Item context menu はImport Target設定済みの配下fileをすべて、file cardのcontext menuはそのfileのImport TargetをFile Treeと同じ経路で取り込む。Item context menuからは配下の全file、file cardのcontext menuからはそのfileのimport済みGUIDをProject Windowでハイライトする。対象種別として成立するcontext actionは常に同じ順序で表示し、現在実行できないactionだけを無効化する。その対象では常に実行できないactionは表示しない。右クリックした対象自身が現在のハイライト対象なら「ハイライトを解除」、それ以外なら「ハイライト」を表示し、別の対象をハイライトすると既存のハイライトを置き換える。Project Windowではハイライト中のassetを選択したcontext menuの`AssetManager/Clear Highlight`からも解除できる。対象assetの親folderも展開経路としてハイライトし、次のハイライト操作または解除操作までsession内で保持する
- Project Window では import 済み folder GUID に対応する AssetManager item の thumbnail を folder icon として表示する。適用対象 folder が親子関係にある場合は最上位の folder だけを表示対象とし、子孫 folder へ同じ thumbnail を重ねない。複数 Item が同じ GUID に対応する場合は最後に import した関連付けを優先し、user setting `assetManager.showProjectWindowIcons` で表示だけを切り替える。AssetManager または FolderStyle が full-size icon を描画した folder では FolderContentOverlay の内容種別 icon を抑制する。描画 callback 中は DB、filesystem、thumbnail の読み込みを行わない
- AssetManager から import した asset は既定で保護し、Project Window と Inspector の通常操作、保存、移動、名前変更、削除から元 asset を変更できないようにする。保護 folder の子孫も同じ保護範囲とする。保護中の asset を Inspector で開くと通常 Inspector に半透明の警告を重ね、Variant または copy の作成を促す。警告には保護解除と警告解除を表示する。警告解除は同じ window で同じ asset を表示している間だけ維持し、保護は解除しない。Project Window には保護専用 icon を追加しない。参照、build、保護範囲外への copy は許可する。保護解除は import GUID 単位で保存し、同じ GUID の再 import でも維持する。AssetManager 自身の再 import 中だけ対象 file の保護を一時解除し、完了または失敗後に現在の DB 状態から再適用する
