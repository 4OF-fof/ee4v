...
- Collection 行の右クリック context menu から名前変更と削除を行える。名前変更 popup は右クリック位置を基準に開き、Smart Collection では名前とアイコンを同時に変更できる。Smart Collection では条件変更も表示し、右クリック位置を基準に開く編集 popup へ保存済みの match mode と条件一覧を復元する。削除前には確認を表示し、対象 Collection と配下の子孫 Collection を削除する。Collection への Item 所属は解除するが Item 本体は削除しない
