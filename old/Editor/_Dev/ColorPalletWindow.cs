    ...
    public class ColorPaletteWindow : EditorWindow {
        ...
        private void OnGUI() {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar)) {
                ...
                if (GUILayout.Button("Refresh / Analyze", EditorStyles.toolbarButton, GUILayout.Width(120))) Analyze();
            }
            ...
            if (_colorDefinitions.Count > 0) {
                ...
                using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox)) {
                    foreach (var def in _colorDefinitions) DrawColorItem(def);
                }
            ...
            if (_styleColorDefinitions.Count > 0) {
                ...
                using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox)) {
                    foreach (var def in _styleColorDefinitions) DrawColorItem(def);
                }
            }
            ...
            using (new EditorGUILayout.VerticalScope(GUI.skin.box)) {
                using (new EditorGUILayout.HorizontalScope()) {
                    ...
                    using (new EditorGUILayout.VerticalScope()) {
                        using (new EditorGUILayout.HorizontalScope()) {
                            EditorGUILayout.SelectableLabel(colorDef.Name, EditorStyles.boldLabel,
                                GUILayout.Height(18));
                            GUILayout.FlexibleSpace();
        ...
        private void Analyze() {
            ...
            try {
                ...
                _colorDefinitions = allDefs.Where(d => fields.First(f => f.Name == d.Name).FieldType == typeof(Color))
                    .ToList();
                _styleColorDefinitions = allDefs
                    .Where(d => fields.First(f => f.Name == d.Name).FieldType == typeof(StyleColor)).ToList();
                ...
                for (var i = 0; i < totalFiles; i++) {
                    ...
                    EditorUtility.DisplayProgressBar("Analyzing Colors", $"Scanning {fileName}...",
                        (float)i / totalFiles);
                    ...
                    for (var lineIndex = 0; lineIndex < lines.Length; lineIndex++) {
                        ...
                        foreach (var def in allDefs) {
                            ...
                            if (isColorPresetFile) {
                                if (line.Contains(def.Name) && !line.Contains("public static") &&
                                    !line.Contains($"{def.Name} =")) isMatch = true;
                            }
                            else {
                                if (line.Contains($"ColorPreset.{def.Name}")) isMatch = true;
                            }
