...
namespace Ee4v.Testing.Infrastructure.Unity
{
    internal sealed class FeatureTestRunnerService : IDisposable
    {
        ...
        public bool TryRun(FeatureTestDescriptor descriptor, out string errorMessage)
        {
            if (descriptor == null)
            {
                errorMessage = "testing.runner.error.descriptorRequired";
                return false;
            }
        ...
        public bool TryRunAll(IReadOnlyList<FeatureTestDescriptor> descriptors, out string errorMessage)
        {
            ...
            {
                errorMessage = "testing.runner.error.noSuites";
                return false;
            }
        ...
        private bool TryStartRun(IReadOnlyList<FeatureTestDescriptor> descriptors, out string errorMessage)
        {
            if (IsRunInProgress)
            {
                errorMessage = "testing.runner.error.alreadyRunning";
                return false;
            }
            ...
            foreach (var descriptor in descriptors)
            {
                ...
                record.Message = "testing.runner.requested";
                record.MessageArgument = string.Empty;
            ...
            foreach (var descriptor in activeRun.Descriptors)
            {
                ...
                if (!byAssembly.TryGetValue(NormalizeAssemblyName(descriptor.AssemblyName), out var assemblyResult))
                {
                    ...
                    record.Message = activeRun.HasStarted
                        ? "testing.runner.missingResult"
                        : "testing.runner.missingStart";
                    record.MessageArgument = string.Empty;
                ...
                record.Message = BuildResultMessage(
                    descriptor,
                    assemblyResult);
                record.MessageArgument = string.Empty;
        ...
        private static string BuildResultMessage(FeatureTestDescriptor descriptor, ITestResultAdaptor assemblyResult)
        {
            ...
            switch (ToStatus(assemblyResult))
            {
                case FeatureTestRunStatus.Passed:
                    return "testing.runner.passed";
                case FeatureTestRunStatus.Skipped:
                    return "testing.runner.skipped";
                case FeatureTestRunStatus.Inconclusive:
                    return "testing.runner.inconclusive";
        ...
        private static string BuildDetailedResult(
            FeatureTestDescriptor descriptor,
            FeatureTestRunRecord record,
            ITestResultAdaptor assemblyResult)
        {
            if (!HasProblemResult(record))
            {
                return string.Empty;
            }
        ...
        private static string BuildFailureDetailsSection(FeatureTestDescriptor descriptor, ITestResultAdaptor assemblyResult)
        {
            ...
            for (var i = 0; i < leafResults.Count; i++)
            {
                ...
                {
                    "[" + FormatStatusLabel(status) + "] " + ResolveLeafTitle(descriptor, leaf)
                };

                if (!string.IsNullOrWhiteSpace(leaf.Message))
                {
                    lines.Add(leaf.Message.Trim());
                }
        ...
        private static string ResolveLeafTitle(FeatureTestDescriptor descriptor, ITestResultAdaptor leaf)
        {
            if (leaf == null)
            {
                return string.Empty;
            ...
            var fullName = leaf.Test != null
                ? leaf.Test.FullName ?? string.Empty
                : string.Empty;
            if (descriptor != null && descriptor.TestCases != null)
            {
                for (var i = 0; i < descriptor.TestCases.Count; i++)
                {
                    var testCase = descriptor.TestCases[i];
            ...
            return leaf.Test != null
                ? leaf.Test.Name ?? string.Empty
                : string.Empty;
        }

        private static string FormatStatusLabel(FeatureTestRunStatus status)
        {
            switch (status)
            {
        ...
        private void OnRunStarted()
        {
            ...
            foreach (var descriptor in _activeRun.Descriptors)
            {
                var record = GetRecord(descriptor.FeatureScope);
                record.Status = FeatureTestRunStatus.Running;
                record.Message = "testing.runner.running";
                record.MessageArgument = string.Empty;
                record.DetailedResult = string.Empty;
                record.CaseDetails.Clear();
            }
        ...
        private void OnTestStarted(ITestAdaptor test)
        {
            ...
            foreach (var descriptor in _activeRun.Descriptors)
            {
                var record = GetRecord(descriptor.FeatureScope);
                record.Status = FeatureTestRunStatus.Running;
                record.Message = string.IsNullOrWhiteSpace(
                    _activeRun.LastTestName)
                        ? "testing.runner.running"
                        : "testing.runner.runningTest";
                record.MessageArgument = _activeRun.LastTestName;
                record.DetailedResult = string.Empty;
                record.CaseDetails.Clear();
            }
        ...
        private void OnEditorUpdate()
        {
            ...
            if (!_activeRun.HasStarted && now - _activeRun.RequestedAtUtc >= TimeSpan.FromSeconds(5))
            {
                ...
                {
                    var record = GetRecord(descriptor.FeatureScope);
                    record.Message = "testing.runner.pending";
                    record.MessageArgument = string.Empty;
                }
            }
            ...
            {
                FailActiveRun("testing.runner.startTimeout");
                return;
            }

            if (_activeRun.HasStarted && now - _activeRun.LastHeartbeatUtc >= RunHeartbeatTimeout)
            {
                FailActiveRun("testing.runner.heartbeatTimeout");
                return;
            }
        ...
        private void SaveState()
        {
            var state = new PersistedState
            {
                records = _records.Select(pair => new PersistedRecord
                {
                    ...
                    messageArgument = pair.Value.MessageArgument,
                    detailedResult = pair.Value.DetailedResult,
            ...
            if (state.records != null)
            {
                for (var i = 0; i < state.records.Length; i++)
                {
                    ...
                    _records[persisted.featureScope] = new FeatureTestRunRecord
                    {
                        ...
                        MessageArgument =
                            persisted.messageArgument ?? string.Empty,
                        DetailedResult = persisted.detailedResult ?? string.Empty,
