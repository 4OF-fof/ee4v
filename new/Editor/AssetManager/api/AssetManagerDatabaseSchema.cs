...
namespace Ee4v.AssetManager.Api
{
    internal static partial class AssetManagerDatabase
    {
        ...
        private static void EnsureSchema(SQLiteConnection connection)
        {
            ...
            connection.Execute("CREATE TABLE IF NOT EXISTS item_tag(item_info_id TEXT NOT NULL REFERENCES item_info(id) ON DELETE CASCADE, tag_info_id TEXT NOT NULL REFERENCES tag_info(id) ON DELETE CASCADE, created_at TEXT NOT NULL, PRIMARY KEY(item_info_id, tag_info_id))");
            ...
            connection.Execute("CREATE TABLE IF NOT EXISTS collection_collection(parent_collection_id TEXT NOT NULL REFERENCES collection_info(id) ON DELETE CASCADE, child_collection_id TEXT NOT NULL UNIQUE REFERENCES collection_info(id) ON DELETE CASCADE, created_at TEXT NOT NULL, CHECK(parent_collection_id != child_collection_id), PRIMARY KEY(parent_collection_id, child_collection_id))");
            connection.Execute("CREATE TABLE IF NOT EXISTS item_collection(item_info_id TEXT NOT NULL REFERENCES item_info(id) ON DELETE CASCADE, collection_info_id TEXT NOT NULL REFERENCES collection_info(id) ON DELETE CASCADE, created_at TEXT NOT NULL, PRIMARY KEY(item_info_id, collection_info_id))");
            ...
            connection.Execute("CREATE TABLE IF NOT EXISTS file_info(id TEXT PRIMARY KEY, item_info_id TEXT NOT NULL REFERENCES item_info(id) ON DELETE CASCADE, file_name TEXT NOT NULL, extension TEXT, size_bytes INTEGER, download_id INTEGER, is_primary INTEGER NOT NULL CHECK(is_primary IN (0, 1)), lifecycle TEXT NOT NULL CHECK(lifecycle IN ('active', 'archived')), created_at TEXT NOT NULL, updated_at TEXT NOT NULL)");
            ...
            connection.Execute("CREATE UNIQUE INDEX IF NOT EXISTS unique_file_info_primary ON file_info(item_info_id) WHERE is_primary = 1");
            connection.Execute("CREATE TABLE IF NOT EXISTS file_dependency(dependent_file_info_id TEXT NOT NULL REFERENCES file_info(id) ON DELETE CASCADE, dependency_file_info_id TEXT NOT NULL REFERENCES file_info(id) ON DELETE CASCADE, dependency_type TEXT NOT NULL CHECK(dependency_type IN ('requires')), created_at TEXT NOT NULL, CHECK(dependent_file_info_id != dependency_file_info_id), PRIMARY KEY(dependent_file_info_id, dependency_file_info_id, dependency_type))");
            connection.Execute("CREATE TABLE IF NOT EXISTS file_import_target(id TEXT PRIMARY KEY, file_info_id TEXT NOT NULL REFERENCES file_info(id) ON DELETE CASCADE, relative_path TEXT NOT NULL, is_directory INTEGER NOT NULL CHECK(is_directory IN (0, 1)), created_at TEXT NOT NULL, updated_at TEXT NOT NULL)");
