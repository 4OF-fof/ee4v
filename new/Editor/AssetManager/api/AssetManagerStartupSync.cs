...
namespace Ee4v.AssetManager.Api
{
    ...
    internal static class AssetManagerStartupSync
    {
        ...
        static AssetManagerStartupSync()
        {
            EnsureInitialized();
        }
        ...
        private static PreparedStartupSync Prepare(bool checkBlm, string blmPath, bool checkEagle, string eaglePath)
        {
            return new PreparedStartupSync(
                checkBlm ? AssetManagerDatabase.PrepareBlmSync(new BlmSyncRequest(blmPath)) : null,
                checkEagle ? AssetManagerDatabase.PrepareEagleSync(new EagleSyncRequest(eaglePath)) : null);
        }
        ...
        private static void Apply(PreparedStartupSync prepared, IDisposable activity)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    results.Add(AssetManagerDatabase.ApplyPreparedBlmSync(prepared.Blm, true));
                }

                if (prepared.Eagle != null && prepared.Eagle.Preview.HasChanges)
                {
                    results.Add(AssetManagerDatabase.ApplyPreparedEagleSync(prepared.Eagle, true));
                }
        ...
        {
            internal PreparedStartupSync(PreparedBlmSync blm, PreparedEagleSync eagle)
            {
                Blm = blm;
                Eagle = eagle;
            }

            internal PreparedBlmSync Blm { get; }

            internal PreparedEagleSync Eagle { get; }
