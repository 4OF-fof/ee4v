...
- `UiTextFactory.Create(...)` に渡す typography 用 class 名の入口になる
- `TypographyStyleResolver` と組み合わせて、どのテキストがどの style で描画されるかを固定する

そのため、`TypographyStyleResolver` で解決する text 用 class 名は string 直書きせず、`UiClassNames` に定数を追加して使う。

一方で、`TypographyStyleResolver` で使わない class は原則 component 内に閉じる。

- 構造用 class
- state modifier 用 class
- その component の `.uss` でしか使わない非 text class

これらは `private const string` などで component 内に持ち、`UiClassNames` へは追加しない。
...
- `Label` を直接使っていないか
- text 表示を `UiTextFactory` 経由にし、typography が必要な class 名だけを `UiClassNames` に追加したか
